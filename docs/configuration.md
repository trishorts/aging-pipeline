...
| `contaminant_exclude` | path | `data/contaminant_panel_exclude_v1.tsv` | A TSV with an `accession` column: entries dropped from the contaminant panel before searching (relative paths resolve against the pipeline folder). The shipped list removes 41 human proteins of a spike-in standard (UPS1/UPS2-like: PRDX1, SOD1, CAT, CKM, MAPT and others) that are not reagents. In mouse and rat searches they captured native proteins' peptides and labelled them contaminant. The reduced panel is written to `<work_root>/db/contaminants/`, named by the hash of both inputs, and recorded as `contaminant_panel` in provenance. Every listed accession must be in the panel, or the search refuses. Omit the key to search the panel unchanged |
...
| `gptmd_extra_mods` | list of `"Category<TAB>Name on Target"` | `["Trypsin Digested<TAB>GG (Ubiquitination Site) on K"]` | Appended to the GPTMD task's modification list. MetaMorpheus's default list has no per-protease remnant category, so without this the diGly (GG) remnant of ubiquitin, NEDD8 and ISG15 is never discovered. Each name is checked against the pinned MetaMorpheus's `Mods/*.txt`, and an unknown one stops the stage. Caveat: two carbamidomethylations on K weigh the same as GG (114.043 Da), so GG calls in iodoacetamide-alkylated samples need care |
