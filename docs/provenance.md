...
| `intensity_share_definition` | `QuantProject DEF-QC-9 v3.5` (both row sets at `Protein QValue` ≤ 0.01) since 2026-09-24; `v2` (no filter) in searches before that. Intensity metrics are QuantProject's; this is their contaminant intensity fraction (PSI MS:4000177) |
| `intensity_share_upper_per_file`, `_upper_median`, `_upper_max` | The **upper bound** on the same per-file share: it also counts every `T` protein group whose members are all in BOTH the contaminant panel and the searched proteome (`aging DEF-CONTAM-INT-SHARED v1`). MetaMorpheus's `TCAmbiguity = RemoveContaminant` makes those proteins `T` (116 in the human proteome, human serum albumin and keratins among them; 26 mouse; 0 rat), so `intensity_share_*` cannot see them and is the **lower bound**. The true share lies between; neither is "the" contamination, because keratin or albumin can be the biology (skin, serum) and still partly contamination. No flag uses the upper bound |
| `intensity_share_upper_definition` | `aging DEF-CONTAM-INT-SHARED v1` |
...
| `aging:DEF-PSM-1PCT-RUN v1` | **run** | `results.txt`'s per-file line `<file> - Target PSMs with q-value <= 0.01`. MetaMorpheus 1.1.11 **recomputes PSM FDR on that file's PSMs alone** (`PostSearchAnalysisTask`, "FDR Analysis is performed again for each file") and counts targets, not decoys, not contaminants, with `QValue <= 0.01 AND QValueNotch <= 0.01` (`FilteredPsms`, filter type q-value, which is what the printed wording "q-value" rather than "pep q-value" certifies). **Never summed**: the per-file lines do not add up to `DEF-PSM-1PCT` |
| `aging:DEF-PEPTIDE-1PCT-RUN v1` | **run** | The per-file `Target peptides with q-value <= 0.01` line: the same per-file FDR at peptide level |
| `aging:DEF-PROTEINGROUP-1PCT-RUN v1` | **run** | The per-file `Target protein groups with q-value <= 0.01` line. The groups are the **whole search's** groups (`ConstructSubsetProteinGroup`, so membership comes from whole-search parsimony), **re-scored and re-FDR'd on this file's PSMs alone**; counted at `QValue <= 0.01` and not decoy, so contaminant groups are counted. Neither qc 010's meaning 1 nor meaning 2 exactly: whole-search composition, per-file protein FDR |
| `aging:DEF-PSM-1PCT-INFILE v1` | **run** | The population behind every PSM-derived per-file metric and distribution in the qc payload: rows of `AllPSMs.psmtsv` from this file with `QValue <= 0.01`, `QValue Notch <= 0.01` and an unambiguous `Notch`. `AllPSMs.psmtsv` is written **before** the per-file FDR recalculation, so these are whole-search q-values: the dataset's accepted PSMs that came from this file, **not** the file's own `DEF-PSM-1PCT-RUN` set. Target-only for all metrics except the contaminant share. The file-side reproduction of `DEF-PSM-1PCT` carries the error bound in [ID rate](#id-rate) |
...
| `aging:DEF-CONTAM-PSM-RUN v2` | **run** | Contaminant PSMs ÷ (target + contaminant) PSMs in the `DEF-PSM-1PCT-INFILE` population, decoys excluded, **within one file**. **v1** (payloads built before 2026-09-23) filtered on `QValue <= 0.01` alone, without the notch q-value or the ambiguous-notch exclusion, so it described a slightly wider set of PSMs than MetaMorpheus accepts (qc 010 QC-Q12). v1 values are kept as they were produced (D41). An ambiguous `C\|T` counts as not-contaminant and stays in the denominator. This is **not** `DEF-CONTAM-PSM v1` measured per file: that one is a dataset figure, and the register's own rule is that a number is stored at the grain it was measured at. A per-file share is a different quantity and carries a different ID, for the same reason a dataset-level contaminant intensity share is not `DEF-QC-9` rolled up |
| `aging:DEF-CONTAM-INT-SHARED v1` | **run** | The **upper bound** beside DEF-QC-9 (the lower): per file, over protein groups at `Protein QValue` <= 0.01, (apex intensity of `C` groups + `T` groups whose every member accession is in both the contaminant panel and the searched proteome) / (apex intensity of all `T` + `C` groups). A group mixing shared and non-shared members is not counted. Reported beside DEF-QC-9, never instead of it (D53) |
| `QuantProject:DEF-QC-9 v3.5` | **run** | Contaminant ÷ (target + contaminant) protein-group apex intensity, **per file**, over groups at `Protein QValue` ≤ 0.01. **v2** (searches and payloads before 2026-09-24) applied no protein-FDR filter; data keep the version that produced them. The median/min/max this pipeline also records are named as *summaries of the per-run values*, and are not a dataset-level measurement of the same quantity |
